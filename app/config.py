...
class AppConfig:
    ...
    UPLOAD_FOLDER: str = os.getenv('UPLOAD_FOLDER', '/tmp/uploads' if os.getenv('VERCEL') else os.path.join(os.path.dirname(os.path.dirname(__file__)), 'uploads'))
    AUDIO_FOLDER: str = os.getenv('AUDIO_FOLDER', '/tmp/audio' if os.getenv('VERCEL') else os.path.join(os.path.dirname(os.path.dirname(__file__)), 'app', 'static', 'audio'))
